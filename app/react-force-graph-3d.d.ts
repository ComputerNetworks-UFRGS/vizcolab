...
declare module 'react-force-graph-3d' {
    ...
    > {
        ...
        ): ForceFn<NodeType> | undefined;
        d3Force(
            forceName: 'link' | 'charge' | 'center' | string,
            forceFn: ForceFn<NodeType>,
        ...
        // Utility
        getGraphBbox(nodeFilter?: (node: NodeType) => boolean): {
            x: [number, number];
            y: [number, number];
            z: [number, number];
        };
    ...
    >(
        props: ForceGraphProps<NodeType, LinkType> & {
            ref?: React.MutableRefObject<
                ForceGraphMethods<NodeType, LinkType> | undefined
            >;
        },
    ) => React.ReactElement;

    // Change this line
    interface ForceGraph extends FCwithRef {}

    // Add this line
    declare const ForceGraph3D: ForceGraph;
